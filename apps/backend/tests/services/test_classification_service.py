...
def get_classification_details_success():
    ...
    assert classification_service.get_classification_details(ClassificationEnum.ROLE) == defails

def get_classification_details_failure():
    with pytest.raises(AssertionError) as exc_info:
        classification_service.get_classification_details("INVALID")
...
def get_classification_detail_success():
    ...
    assert classification_service.get_classification_detail(ClassificationEnum.ROLE, 1) == detail

def get_classification_detail_failure():
    with pytest.raises(AssertionError) as exc_info:
        classification_service.get_classification_detail(ClassificationEnum.ROLE, '9999')
